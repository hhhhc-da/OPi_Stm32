...
#define MOTION_DRIVER_TARGET_MSP430//官方案例MSP430驱动，这里未修改宏名
...
// 函数接口区，需要修改
#define i2c_write   MPU6050_writeDataByLength
#define i2c_read    MPU6050_readDataByLength
...
int mpu_init(void)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;
    delay_ms(100);
	DEBUG_printf("Reset device OK\r\n");

    /* Wake up chip. */
    data[0] = 0x00;
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;
	DEBUG_printf("Wake up chip OK\r\n");

#if defined MPU6050
    /* Check product revision. */
    if (i2c_read(st.hw->addr, st.reg->accel_offs, 6, data))
        return -1;
	DEBUG_printf("Check product revision...\r\n");
...
//        reg_int_cb(int_param);
	...
	DEBUG_printf("setup_compass mode\r\n");
    if (mpu_set_bypass(0))
        return -1;
	DEBUG_printf("Already disabled by setup_compass\r\n");
#endif
...
#define LOAD_CHUNK  (16)
	...
	DEBUG_printf("检测是否只初始化了一次\r\n");
    if (st.chip_cfg.dmp_loaded)
        /* DMP should only be loaded once. */
        return -1;

	DEBUG_printf("检测firmware\r\n");
    if (!firmware)
        return -1;
	...
	DEBUG_printf("Set program start address...\r\n");
	...
	DEBUG_printf("Set program start address OK\r\n");
...
u8 run_self_test(void)
{
	...
	// 要让他得数为 0x3
	DEBUG_printf("mpu_run_self_test result: %d\r\n",result);
...
u8 mpu_dmp_init(void)
{
	...
	DEBUG_printf("初始化结果: %x\r\n",result);
	
	if(result==0)	//初始化MPU6050
	{	 
		// res=mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL);//设置所需要的传感器（角加速度+加速度传感器）
		res=mpu_set_sensors(INV_XYZ_GYRO);
		if(res)return 1; 
		// res=mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL);
		res=mpu_configure_fifo(INV_XYZ_GYRO);//设置FIFO
		if(res)return 2; 
		...
		if(res)return 5; 
		res=dmp_enable_feature(	DMP_FEATURE_6X_LP_QUAT 		|  	// 启用低功耗四元数输出
								DMP_FEATURE_SEND_CAL_GYRO 	|   // 启用校准后的陀螺仪数据输出
								DMP_FEATURE_GYRO_CAL);			// 启用陀螺仪校准功能
		if(res)return 6; 
